...
class openOrders():
    ...
    '''
    def processOpenOrders(ui,config,podaci,tableIndex):
        ...
        for podatak in podaci:
            ...
            openOrders.setColortoRow(table,table.rowCount()-1,QtGui.QColor(77, 210, 219))
    '''
...
class openOrdersThread(QtCore.QThread):
    output = QtCore.pyqtSignal(object,int);
        ...
        while True:
            for tableIndex in range(0,len(self.config.clients)):
                for pair in self.config.pairs:
                    ...
                    self.orderProccesing(trenutnaTabela,tableIndex,podaci,pair);
                    self.config.weight+=3;
                    await asyncio.sleep(1);
                marginOrders = self.config.clients[tableIndex].get_open_margin_orders();
                self.orderProccesing(trenutnaTabela,tableIndex,marginOrders,pair);
                self.config.weight+=10;
                await asyncio.sleep(1);
                
    def orderProccesing(self,trenutnaTabela,tableIndex,podaci,pair):
        
        self.checkForTheExecutedData(trenutnaTabela,tableIndex,podaci,pair);
        ...
        if len(podaci)>0:
            ...
            self.output.emit(podaci,tableIndex)
    ...
    '''
    def checkForTheExecutedData(self,trenutnaTabela,tableIndex,podaci,pair):
        for vecPostojaciPodatak in trenutnaTabela:
            if vecPostojaciPodatak["symbol"] == pair:
                if not any(podatak['orderId'] == vecPostojaciPodatak['orderId'] for podatak in podaci):
                    ...
                    print(f"PODATAK IZBRISAN {vecPostojaciPodatak['symbol']} {vecPostojaciPodatak['orderId']}");
    '''
